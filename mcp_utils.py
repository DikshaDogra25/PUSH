...
def parse_mcp_output(
    ...
    final_plan = DealSchedulePlan.deal_schedule_plan(
        product_list, annotated_purchase, tag_rows, sale_deals, used_sale_product
    )
...
def _trans_type(row: Any) -> str:
    ...
    if not isinstance(row, dict):
        return ""
    for key in ("TransactionType", "TransType", "TransactionTypeName"):
        v = str(row.get(key, "") or "").strip().lower()
        if v.startswith("sale") or v == "s":
            return "Sale"
        if v.startswith("purchase") or v == "p":
            return "Purchase"
    return ""
...
def _extract_deal_rows(scheduled_output: Any) -> list[dict]:
    ...
    Tolerant of both a flat list of dicts and a list of result sets (like
    ``get_purchase_deal_data``); prefers the set whose rows carry deal info.
    """
    if not scheduled_output:
        return []
    if isinstance(scheduled_output, list) and scheduled_output and isinstance(scheduled_output[0], dict):
        return scheduled_output
    if isinstance(scheduled_output, list):
        fallback: list[dict] = []
