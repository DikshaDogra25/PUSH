...
def parse_mcp_output(
    ...
    # Drop tags in terminal / non-deliverable TagStatus states at the VERY start so
    # the excluded tags never contribute to display, aggregation, or allocation.
    tag_rows = _filter_excluded_tag_status(purchase_output[2])
    ...
    # Same TagStatus filter applied (the scheduled set is a tag+deal join).
    scheduled_deals = _filter_excluded_tag_status(_extract_deal_rows(scheduled_output))
...
# Terminal / non-deliverable tag states (per trader request) that must never appear
# in the plan, aggregation, or any downstream calculation. Matched case-insensitively
# against the Tags table's TagStatus column.
_EXCLUDED_TAG_STATUSES = frozenset(
    {"REJECTED", "UNDELIVERED", "DEAD", "WITHDRAWN", "CANCELLED", "DENIED", "EXPIRED"}
)


def _filter_excluded_tag_status(rows: Any) -> list:
    """Drop rows whose TagStatus is in _EXCLUDED_TAG_STATUSES. Rows without a
    TagStatus column (or non-dict rows) are kept unchanged, so this is a no-op until
    ...
    for r in rows:
        if isinstance(r, dict):
            status = str(r.get("TagStatus", "") or "").strip().upper()
            if status in _EXCLUDED_TAG_STATUSES:
                continue
        out.append(r)
    return out
