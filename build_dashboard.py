...
function applyResultAllocationOverride() {
  ...
  // NEW (graph variations): available (unallocated) + used/scheduled A/B per source.
  const __av = (RESULT_ALLOCATION.available || {}), __us = (RESULT_ALLOCATION.used || {});
  (__av.ProductA || []).forEach(function (x) { (byCP[(x.CP||'').trim()] = byCP[(x.CP||'').trim()] || {}).avA = __resAllocToArr(x); });
  (__av.ProductB || []).forEach(function (x) { (byCP[(x.CP||'').trim()] = byCP[(x.CP||'').trim()] || {}).avB = __resAllocToArr(x); });
  (__us.ProductA || []).forEach(function (x) { (byCP[(x.CP||'').trim()] = byCP[(x.CP||'').trim()] || {}).usA = __resAllocToArr(x); });
  (__us.ProductB || []).forEach(function (x) { (byCP[(x.CP||'').trim()] = byCP[(x.CP||'').trim()] || {}).usB = __resAllocToArr(x); });
  STATE.locations.forEach(function (loc) {
    ...
    // available (unallocated) + used/scheduled A/B curves for the chart sub-toggle.
    loc.availableA = (cp.avA || A).map(function (v) { return Math.round(v * 10) / 10; });
    loc.availableB = (cp.avB || B).map(function (v) { return Math.round(v * 10) / 10; });
    loc.usedA = (cp.usA || Array(24).fill(0)).map(function (v) { return Math.round(v * 10) / 10; });
    loc.usedB = (cp.usB || Array(24).fill(0)).map(function (v) { return Math.round(v * 10) / 10; });
...
def _map_tags(unschedule_tags) -> list[dict]:
    ...
    for t in unschedule_tags:
        ...
        scheduled = _s(t.get("IsScheduledTag")).strip().lower() in ("yes", "y", "true", "1")
        # Column mapping confirmed with the user: Source/Sink <- Source/Sink,
        # POR/POD <- GCA/LCA, CPSE <- CPSE, Path <- MarketPath, Schedule # <- ScheduleNumber.
        schedule_num = _s(t.get("ScheduleNumber")) or f"P-{t.get('TagIndex', '')}"
        out.append(
            {
                "id": uid,
                "schedule_num": schedule_num,
                "tag_code": _s(t.get("TagCode")),
                "source": _s(t.get("Source")),  # Source column
                "sink": _s(t.get("Sink")),
                "por": _s(t.get("GCA")),  # GCA -> POR
                "pod": _s(t.get("LCA")),  # LCA -> POD
                "cpse": _s(t.get("CPSE")),
                "path": _s(t.get("MarketPath")),  # MarketPath column
                ...
                # IsScheduledTag drives the Scheduled/Unscheduled display (Tags tab,
                # tag tooltip). Status stays "Unscheduled" so the accept/commit flow
                # is untouched; `scheduled` is the dedicated display flag.
                "status": "Unscheduled",
                "scheduled": scheduled,
                "is_scheduled_tag": _s(t.get("IsScheduledTag")),
...
def _inject_allocation(template: str, allocation, result: dict, sale_products: list[str] | None = None) -> str:
    ...
    template, n = re.subn(
        r"const SALE_DEAL_PRODUCTS = \[\];",
        f"const SALE_DEAL_PRODUCTS = {sp_json};",
        template,
        count=1,
    )
    if n != 1:
