...
class DealSchedulePlan:
    ...
    @staticmethod
    def _deal_matches_tag(deal, sched_no, cp, is_sale) -> bool:
        """Associate a scheduled deal row with a scheduled tag.

        Primary key is ScheduleNumber (shared by the tag and its deals). When the
        deal rows don't carry ScheduleNumber, purchase deals fall back to a
        CounterParty match and sale deals associate by order (consumed once used),
        which is correct for the single-scheduled-tag result set.
        """
        ...
        # No usable join key (no ScheduleNumber on the deal, no CounterParty on the
        # tag): associate by order, consumed once used. Correct for the single
        # scheduled-tag result set; deterministic for the multi-tag case.
        return True
