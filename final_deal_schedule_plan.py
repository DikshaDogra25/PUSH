...
class DealSchedulePlan:
    ...
    def _compute_tag_sale_profiles(tagdata, tot_a, tot_b, used_sale_product=None):
        ...
        TOTAL Product A / B (per hour). The draw order depends on the tag:
          * SCHEDULED tag  -> carve from its own IndexName leg FIRST (a _Product_B
            scheduled tag consumes Product B before A, matching the already-committed
            deal), overflow to the other leg.
          * UNSCHEDULED tag -> Product A first, overflow to Product B.
        ...
        # Scheduled tag -> its committed sale leg (from the DB deal's IndexName),
        # keyed by TagCode, so a scheduled _Product_B tag carves B first.
        leg_by_tagcode: dict[str, str] = {}
        for deal in used_sale_product or []:
            if not isinstance(deal, dict):
                continue
            tc = DealSchedulePlan._clean(deal.get("TagCode"))
            leg = DealSchedulePlan._sale_leg_from_index(deal.get("IndexName"))
            if tc and leg and tc not in leg_by_tagcode:
                leg_by_tagcode[tc] = leg
        ...
        for src, idxs in by_src.items():
            ...
            for i in scheduled + unscheduled:
                tag = tagdata[i]
                # Scheduled _Product_B tag carves B first; everyone else A first.
                b_first = (
                    i in scheduled
                    and leg_by_tagcode.get(DealSchedulePlan._clean(tag.get("TagCode"))) == "B"
                )
                aA, uA, aB, uB = [], [], [], []
                for he in HES:
