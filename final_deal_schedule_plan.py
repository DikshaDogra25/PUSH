...
class Deal_Schedule_Plan:
    ...
    def _deal_mw_list(deal):
        ...
        NOTE: currently UNUSED — sale mw/volume come from the allocation, not the
        deal. Kept for reference / potential reuse.
    ...
    def _sale_row(leg, entry, alloc, sd):
        # mw/volume always come from the allocation (productList); the actual sale
        # deal's MW is intentionally NOT used here.
        mw = alloc
        return {
            "type": "SALE",
            ...
            # mw/volume are the allocation (productList) values (never the deal's MW).
            "mw":     mw,
            ...
            for i, pd in enumerate(purchasedeals):
                ...
                if used:
                    rows.append({
                        ...
                        # NEW: the deal's ACTUAL status from SQL, so the dashboard
                        # stops hard-coding "Planned". Falls back to "Draft" when
                        # the proc doesn't return a status column.
                        "status": (Deal_Schedule_Plan._clean(
                            pd.get("Status") or pd.get("DealStatus") or pd.get("DealState")) or "Draft"),
