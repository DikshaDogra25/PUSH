...
class DealSchedulePlan:
    ...
    def _sale_row(leg, entry, alloc, sd, schedule_status="Unscheduled"):
        ...
        return {
            ...
            "deal_type": DealSchedulePlan._clean(sd.get("DealType")) if sd else "",
    ...
    def deal_schedule_plan(
        ...
        tag_profiles = DealSchedulePlan._compute_tag_sale_profiles(tagdata, tot_a, tot_b)
        # Per-tag PURCHASE available/used profiles (running pool per CounterParty).
        purchase_profiles = DealSchedulePlan._compute_tag_purchase_profiles(tagdata, purchasedeals, used_purchase)
        ...
        for tag_i, tag in enumerate(tagdata):
            ...
            if is_scheduled:
                ...
                for i, sdl in enumerate(used_sale_product):
                    ...
                    srow = DealSchedulePlan._scheduled_sale_row(sdl)
                    DealSchedulePlan._attach_profile(srow, srow.get("leg", ""), tag_profiles.get(tag_i))
                    rows.append(srow)
                ...
                DealSchedulePlan._attach_purchase_profile(rows, purchase_profiles.get(tag_i))
    ...
    def _attach_profile(row, leg, prof):
        """Attach this tag's own available (total_mw = running-remaining-before) and
        used curves for the sale row's leg, so the hover overlays used on available."""
        prof = prof or {}
        if leg == "B":
    ...
    @staticmethod
    def _compute_tag_sale_profiles(tagdata, tot_a, tot_b):
        ...
        TOTAL Product A / B (per hour); each tag draws its own MW need from Product
        A first, overflowing to B. For each tag we record, per hour:
          * available (aA/aB) = the running remaining BEFORE this tag allocated
          * used (uA/uB)      = this tag's own draw from A / B
        Returns { tag_index -> {"aA","uA","aB","uB"} }, each a 24-length list.
        """
        HES = list(range(1, 25))
        ...
        for src, idxs in by_src.items():
            ...
            for i in scheduled + unscheduled:
                ...
                profiles[i] = {"aA": aA, "uA": uA, "aB": aB, "uB": uB}
        return profiles
