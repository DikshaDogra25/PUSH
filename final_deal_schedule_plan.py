...
class DealSchedulePlan:
    ...
    def deal_schedule_plan(
        ...
        tot_a, tot_b, use_a, use_b = DealSchedulePlan._build_total_used_curves(saledeals)
        ...
        for tag in tagdata:
            ...
            if is_scheduled:
                ...
                for i, sdl in enumerate(used_sale_product):
                    ...
                    DealSchedulePlan._attach_hover_curves(
                        srow, srow.get("leg", ""), source, tot_a, tot_b, use_a, use_b
                    )
                    rows.append(srow)
            ...
            if a_entry is not None and sum(saleA) > 0:
                ...
                rowA = DealSchedulePlan._sale_row("A", a_entry, saleA, sdA)
                DealSchedulePlan._attach_hover_curves(rowA, "A", lookup_cp, tot_a, tot_b, use_a, use_b)
                rows.append(rowA)
            ...
            if b_entry is not None and sum(saleB) > 0:
                ...
                rowB = DealSchedulePlan._sale_row("B", b_entry, saleB, sdB)
                DealSchedulePlan._attach_hover_curves(rowB, "B", lookup_cp, tot_a, tot_b, use_a, use_b)
                rows.append(rowB)
    ...
    @staticmethod
    def _attach_hover_curves(row, leg, source, ta, tb, ua, ub):
        """Attach the product's TOTAL (available graph) and USED curves for the
        sale row's leg + source, so the hover can overlay used on total."""
        if leg == "B":
            row["total_mw"] = DealSchedulePlan._curve24(tb, source)
            row["used_mw"] = DealSchedulePlan._curve24(ub, source)
        else:
            row["total_mw"] = DealSchedulePlan._curve24(ta, source)
            row["used_mw"] = DealSchedulePlan._curve24(ua, source)
        return row
