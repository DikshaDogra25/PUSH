...
class DealSchedulePlan:
    ...
    def _scheduled_sale_row(deal):
        ...
        Contract<-ContractName, Zone<-DPName. Counterparty is left BLANK until
        the sale counterparty column is confirmed (candidates DPName/DPSSE/BPAT).
        """
        ...
        return {
            ...
            "counterparty": "",  # sale counterparty column unresolved -> blank
    ...
    def _sale_row(leg, entry, alloc, sd, schedule_status="Unscheduled"):
        ...
        return {
            ...
            "counterparty": (DealSchedulePlan._clean(sd.get("CP")) or "Amazon Energy Services")
            if sd
    ...
    def deal_schedule_plan(
        cls, saledeals, purchasedeals, tagdata, actualsaledeal, used_sale_product=None
    ) -> list[dict[str, Any]]:
        """
        ...
        sale_consumed = [False] * len(used_sale_product)
        ...
        for tag in tagdata:
            ...
            schedule_number = "PENDING"  # f"P-{random.randint(10000000, 99999999)}"
            rows = []
            ...
            # ---------- existing (already-scheduled) SALE deals for this source ----------
            # Match the used sale deals on Source; each is shown once (consumed).
            src_key = source.upper()
            for si, sdeal in enumerate(used_sale_product):
                if sale_consumed[si]:
                    continue
                if DealSchedulePlan._clean(sdeal.get("Source")).upper() != src_key:
                    continue
                sale_consumed[si] = True
                rows.append(DealSchedulePlan._scheduled_sale_row(sdeal))
