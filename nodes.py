...
async def do_format(state: AgentState, config: RunnableConfig) -> dict:
    ...
    if isinstance(last_message, ToolMessage):
        ...
        responsemessage = ""
        # CHANGED: the purchase-deal proc now returns THREE result sets --
        #   data[0] = SaleDealProduct list, data[1] = purchase deals, data[2] = tags.
        # Named locals below so the indices are declared once instead of being
        # scattered through the block.
        sale_deal_products = data[0] if len(data) > 0 else []
        purchase_deals = data[1] if len(data) > 1 else []
        tag_rows = data[2] if len(data) > 2 else []
        ...
        if len(data) < 2:
            ...
            ruledata = AppyRules(tag_rows)
            saveOutputRes(ruledata, "ruledata")
            saveOutputRes(ruledata[0], "ruledtagdata")
            # purchasedeal = get_purchase_deal(ruledata[0], purchase_deals)
            saveOutputRes(purchase_deals, "purchasedeal")
            saveOutputRes(sale_deal_products, "saledealproduct")
            ...
            deal_Schedule_final_plan = Deal_Schedule_Plan().deal_schedule_plan(
                productList, purchase_deals, tag_rows, saledeals)
            finalresult = {"Unschedule Tags": tag_rows,
                           "Purchase Deals": purchase_deals,
                           "Location Agrregation & Allocation": AggregatedList,
                           "FINAL_PLAN": deal_Schedule_final_plan,
                           # NEW: distinct SaleDealProduct values -> Product dropdown
                           # on the SALE rows of the Final Deal Schedule Plan grid.
                           "Sale_Deal_Product_List": sale_deal_products}
